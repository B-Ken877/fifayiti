...
import { getBettingSecret } from "@/lib/auth/secret";
...
function sign(payload: BettorSessionPayload): string {
  ...
  const mac = createHmac("sha256", getBettingSecret()).update(body).digest("hex");
  return `${body}.${mac}`;
}
...
function verify(token: string): BettorSessionPayload | null {
  ...
  const expectedMac = createHmac("sha256", getBettingSecret()).update(body).digest("hex");
